...
QByteArray Server::handle_update_status(QByteArray& req)
{
    int ret;
    Json j(req);
    ...
    if(ret!=0)
    {
        SEND_FAILEDMSG(HC_UPDATESTATUS,HC_UNKNOW);
    }
    QString t = QString::number(time);
    ret = Redis::getInstance()->setHash(username,"time",t);
    if(ret!=0)
    {
        SEND_FAILEDMSG(HC_UPDATESTATUS,HC_UNKNOW);
    }
...
QByteArray Server::handle_updatepos(QByteArray& req)
{
    ...
    if(ret!=0)
    {
        SEND_FAILEDMSG(HC_UPDATEPOS,HC_UNKNOW);
...
QByteArray Server::handle_Login(QByteArray& req)
{
    ...
    if(type==HC_DRIVER) //driver
    {
        ...
        if(ret!=1)
        {
            SEND_FAILEDMSG(HC_LOGIN,HC_NOTUSER);
        }
        Json msg(array[0]);
...
QByteArray Server::handle_Reg(QByteArray& req)
{
    ...
    if(type==HC_DRIVER) //driver
    {
        ...
        {
            SEND_FAILEDMSG(HC_REG,HC_USEREXIST);
        }
        ...
        if(ret!=0)
        {
            SEND_FAILEDMSG(HC_REG,HC_REG_FAILED);
        }
    }else if(type==HC_PASSENGER) //passenger
    {
        username+="P";
